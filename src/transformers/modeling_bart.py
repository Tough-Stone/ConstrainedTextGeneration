...
class BartModel(PretrainedBartModel):
    ...
    def get_encoder_outputs(
        ...
        return_tuple = return_tuple if return_tuple is not None else self.config.use_return_tuple
        print(output_attentions, output_hidden_states, return_tuple)
...
class BartForTextInfill(PretrainedBartModel):
    """
    ...
    Conditional generation example::
        ...
        if encoder_labels is not None:
            ...
            if  self.encoder_loss_type  ==0:
                loss_fct = nn.CrossEntropyLoss()
                encoder_loss = loss_fct(encoder_logits.view(-1, self.config.num_labels), encoder_labels.view(-1))
            # regression loss
            else:
